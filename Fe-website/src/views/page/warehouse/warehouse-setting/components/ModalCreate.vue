    ...
    <a-form @submit.prevent="handleSubmit" class="tw-mb-6">
      <div class="tw-w-full tw-flex tw-items-start tw-justify-between tw-mb-6">
        <div class="tw-basis-1/2 tw-flex tw-flex-col tw-items-start tw-justify-start tw-mr-4">
          <span class="tw-mb-2"
            >{{ translate('WarehouseCode') }}<span class="required-star">*</span></span
          >
          <div v-if="!isEdit" class="tw-w-full">
            <a-input
              :placeholder="translate('WarehouseCode')"
              class="tw-w-full"
            ...
            <ErrorMess
              ...
              :validator="v$.warehouseId.$errors[0]?.$validator"
            />
          </div>
        ...
        <div class="tw-basis-1/2 tw-flex tw-flex-col tw-items-start tw-justify-start">
          <span class="tw-mb-2"
            >{{ translate('warehouseName') }}<span class="required-star">*</span></span
          >
          <a-input
            :placeholder="translate('warehouseName')"
            class="tw-w-full"
            :status="v$.warehouseName.$error ? 'error' : ''"
            v-model:value="v$.warehouseName.$model"
          />
          <ErrorMess
            :params="[64]"
            :title="translate('warehouseName')"
            :validator="v$.warehouseName.$errors[0]?.$validator"
          />
        </div>
      </div>
      <div class="tw-w-full tw-flex tw-flex-col tw-items-start tw-justify-start tw-mb-6">
        ...
        <div class="tw-w-full tw-border tw-p-4 tw-rounded-xl">
          <div class="tw-w-full tw-flex tw-items-start tw-justify-between tw-mb-6">
            <div class="tw-basis-1/2 tw-flex tw-flex-col tw-items-start tw-justify-start tw-mr-4">
              <span class="tw-mb-2"
                >{{ translate('Nation') }}<span class="required-star">*</span></span
              >
              <a-select
                class="tw-w-full"
                :placeholder="translate('Nation')"
                :status="v$.Nation.$error ? 'error' : ''"
                v-model:value="v$.Nation.$model"
              />
              <ErrorMess
                :params="[64]"
                :title="translate('Nation')"
                :validator="v$.Nation.$errors[0]?.$validator"
              />
            </div>
            <div class="tw-basis-1/2 tw-flex tw-flex-col tw-items-start tw-justify-start tw-mr-4">
              <span class="tw-mb-2"
                >{{ translate('Area') }}<span class="required-star">*</span></span
              >
              <a-select
                class="tw-w-full"
                :status="v$.Area.$error ? 'error' : ''"
                v-model:value="v$.Area.$model"
          ...
          <div class="tw-w-full tw-flex tw-flex-col tw-items-start tw-justify-start tw-mr-4">
            <span class="tw-mb-2"
              >{{ translate('Address') }}<span class="required-star">*</span></span
            >
            <a-input
              :placeholder="translate('Address')"
              class="tw-w-full"
              :status="v$.describe.$error ? 'error' : ''"
      ...
      <div class="tw-flex tw-items-start tw-justify-between">
        <div
          class="tw-basis-1/2 tw-flex tw-flex-col tw-items-start tw-justify-center tw-mr-4 tw-w-full"
        >
          <div class="tw-mb-2 tw-flex tw-items-center tw-justify-between tw-w-full">
            <span
              >{{ translate('Acreage') }}(m<sup>2</sup>)<span class="required-star">*</span></span
            >
            <span class="tw-opacity-70"
              >(A = {{ translate('Length') }} x {{ translate('Width') }})</span
            >
          </div>
          <a-input
            :placeholder="translate('Acreage')"
            class="tw-w-full"
            :status="v$.Acreage.$error ? 'error' : ''"
            v-model:value="v$.Acreage.$model"
          />
          <ErrorMess
            :params="[64]"
            :title="translate('Acreage')"
            :validator="v$.Nation.$errors[0]?.$validator"
          />
        </div>
        <div class="tw-basis-1/2 tw-flex tw-flex-col tw-items-start tw-justify-center tw-w-full">
          <div class="tw-mb-2 tw-flex tw-items-center tw-justify-between tw-w-full">
            <span
              >{{ translate('Tankage') }}(m<sup>3</sup>)<span class="required-star">*</span></span
            >
            <span class="tw-opacity-70">(V = A x H)</span>
          </div>
          <a-input
            :placeholder="translate('Tankage')"
            class="tw-w-full"
            :status="v$.Tankage.$error ? 'error' : ''"
            v-model:value="v$.Tankage.$model"
          />
          <ErrorMess
            :params="[64]"
            :title="translate('Tankage')"
            :validator="v$.Nation.$errors[0]?.$validator"
          />
        </div>
      </div>
...
watch(
  ...
  (val) => {
    ...
    ;(state.warehouseId = val.warehouseId),
      (state.warehouseName = val.warehouseName),
      (state.Nation = val.Nation),
      (state.Area = val.Area),
      (state.describe = val.describe),
      (state.Acreage = val.Acreage),
      (state.Tankage = val.Tankage)
  },
  {
    deep: true
  }
